...
from rhea_peeyush_airport_transfers import models
...
class IndexView(View):
    ...
    def get(self, request):
        ...
        return JsonResponse({"status": "success", "message": "Rhea Peeyush App Running"})
...
class BuyView(View):
    ...
    def post(self, request):
        ...
        sale.save()
        success_url = f"{HOST}/rhea-peeyush/success/{sale.id}?from={from_host}"
        ...
        if vip_code_found or price == 0:
            ...
            template_path = os.path.join(
                current_folder, "templates", "rhea_peeyush_airport_transfers", "mail.html"
            )
            tools.send_sucess_mail(
                [
                    "Rhea & Peeyush Airport Transfer",
                    f"(#{sale.id}) Rhea & Peeyush Airport Transfer",
                ],
                template_path,
                sale.id,
                sale.name,
