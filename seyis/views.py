...
def __sort_data__(data: list, reverse: bool = False):
    """Sort data serializable by name field
...
class SalesView(View):
    """Save sale data and send confirmation email"""
    ...
    def post(self, request):
        ...
        if not (name and last_name and details and email):
            return JsonResponse(
                {
                    "status": "error",
        ...
        template_path = os.path.join(
            current_folder, "templates", "seyis", "mail.html"
        )
        tools.send_sucess_mail(
            [
                "Voucher Seyis Airport Transfer",
                f"(#{sale.id}) Seyis Airport Transfer",
            ],
            template_path,
            sale.id,
            sale.name,
