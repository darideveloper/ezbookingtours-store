...
def widget(request, location, tour):
    ...
    if request.method == "GET":
        ...
        times = list(
            map(lambda tour_time: tour_time.time_start.strftime("%H:%M"), tour_times)
        )
        ...
        for pick_up in pick_ups:
            ...
            hotel_text = (
                hotel.name if not hotel.address else hotel.name + " - " + hotel.address
            )
    ...
    # Process form in post
        ...
        stripe_description = (
            f"Tour de ezbookingtours.com, en la fecha {tour_date} y hora {tour_time}. "
        )
        if pick_up:
        ...
        res = requests.post(
            "https://services.darideveloper.com/stripe-api/", json=request_json
        )
        res_data = res.json()
...
def test_email(request):
    """View to test email credentials"""
