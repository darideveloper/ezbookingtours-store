...
from digitalrealty import models
...
class SaleView(View):
    ...
    def post(self, request):
        ...
        json_data["url_success"] = f"{HOST}/digitalrealty/sale/{sale.id}"
        description_text = ""
        for description_key, description_value in description.items():
...
class SaleDoneView(View):
    ...
    def get(self, request, sale_id):
        ...
        context = {
            ...
            "cta": DIGITALREALTY_PAGE,
            "id": sale.id,
            "details": details,
        }

        # Send email
        html_message = render_to_string("digitalrealty/email.html", context)
        plain_message = strip_tags(html_message)

        # Send email to client
        emails_subjects = {
            EMAIL_USER: "New Sale in Digital Realty",
            sale.email: "Digital Realty - Transportation Confirmation",
        }
        for to_email, subject in emails_subjects.items():
            print(f"Sending digitalrealty email to {to_email}")
        ...
        if preview:
            ...
            return redirect(f"{DIGITALREALTY_PAGE}?done=true")
